...
import { updateToPlayGame } from "../service/update";
...
// Define the props for the ToPLayModal component
...
const ToPlayModal = ({ data, isEdit, open, setOpen }: IToPLayModalProps) => {
  ...
  const submit: SubmitHandler<Payload> = async(dataPayload) => {
    console.log("Form data submitted:", dataPayload);
    if (isEdit) {
      await updateToPlayGame(+dataPayload.gameId, dataPayload.duration);
      setOpen(false);
    }
  };
